...
import type { MultiSelectContent } from "../types";

interface MultiSelectProps {
  content: MultiSelectContent;
  onSubmit: (answer: { selectedIndices: number[] }) => void;
  disabled?: boolean;
...
export function MultiSelect({
  ...
  const correctSet = new Set(content.correctIndices);
  ...
  function handleSubmit() {
    ...
    onSubmit({ selectedIndices: normalize(selected) });
  }

  return (
    <div className="space-y-4" role="group" aria-label="Multi select question">
      <p className="text-lg font-medium" id="ms-question">{content.question}</p>
      <p className="text-sm text-muted-foreground">Select all that apply.</p>

      <div className="space-y-2" aria-labelledby="ms-question">
        {content.options.map((option, idx) => {
          const isSelected = selected.includes(idx);
          const isCorrect = submitted && correctSet.has(idx);
          const isWrongSelection = submitted && isSelected && !correctSet.has(idx);
      ...
      <AnimatePresence>
        {submitted && result && (
          ...
          >
            {result.isCorrect ? "Correct! You found all valid choices." : "Not quite. Review the marked options above."}
          </motion.div>
        )}
      </AnimatePresence>
